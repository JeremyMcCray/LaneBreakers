...
import { TOWER_X } from '../../data/world';
import { newSim, simStep, netWorth } from '../../sim/engine';
import { botThink } from '../bot';
import * as LBBrain from './brain';
import {
  LB_API, LB_BRAIN_CACHE, lbLocalBrainNames, lbBuildAiSelect, lbAiDesc, lbSetAiTier
} from './runtime';
import LB_RECIPES from '../../../ai/recipes.json';
import { G } from '../../app/state';
import { addToast } from '../../render/fx';
import { Store } from '../../app/persistence';

export const LBTrain = {
...
/* ---------------------------------------------------------------------
   ...
   --------------------------------------------------------------------- */
export function lbSimMatch(picks, agents, seed, maxTime){
...
export function lbTrainDraw(){
  ...
  // legend
  c.fillStyle='#5ef0c8'; c.fillText('â–  best', pad.l+8, pad.t+11);
  c.fillStyle='#3d5580'; c.fillText('â–  average', pad.l+52, pad.t+11);
  c.fillStyle='#ffcc55'; c.fillText('â”ˆ beats old bot', pad.l+112, pad.t+11);
}
...
/* ---------------------------------------------------------------------
   ...
   --------------------------------------------------------------------- */
export let _lbUiT = 0;
export function lbTrainUI(throttle){
...
export function lbTrainImport(input){
...
export async function lbTrainPlay(){
  if (!LBTrain.bestGenome) return addToast('Train at least one generation first');
  ...
  LBTrain.pause();
  const { startPractice } = await import('../../app/shell');
  startPractice('1v1');
}
export function lbTrainList(){
...
export function lbTrainOpen(){
  lbBuildRecipeSelect(); lbTrainList(); lbTrainUI(); lbTrainDraw();
}

// Re-export menu AI picker so train save/import can refresh the dropdown.
export { lbBuildAiSelect, lbAiDesc, lbSetAiTier };
