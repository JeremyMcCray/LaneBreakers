/* =====================================================================
   brain.js — THE NEURAL BOT
   =====================================================================

   Read this file if you read nothing else. It is the whole AI.

   ---------------------------------------------------------------------
   THE IDEA, IN PLAIN ENGLISH
   ---------------------------------------------------------------------
   A neural network here is nothing mystical. It is a big pile of numbers
   (we call them "weights") plus a fixed recipe for turning game facts
   into a decision:

       40 numbers describing the situation
           -> multiply by weights, squash, repeat twice
       -> 12 numbers describing what to do

   The first 8 output numbers are the bot's MOOD — farm, push, harass,
   all-in, retreat, deny, siege, reposition. The biggest one wins. The
   last 4 say whether it wants to press Q, W, E or R right now.

   Nobody writes those weights by hand. train.js generates hundreds of
   random ones, makes them play thousands of matches against each other,
   keeps whichever ones won, mutates them slightly, and repeats. That is
   the "learning". It is evolution, not lecturing.

   ---------------------------------------------------------------------
   WHAT THE NETWORK DOES *NOT* DECIDE
   ---------------------------------------------------------------------
   Deliberately, some things stay as ordinary code:

     - the mechanics of landing a last hit (timing a swing is execution,
       not judgement — making the net rediscover it wastes generations)
     - where exactly an ability is aimed (the net decides *whether*, code
       decides *where*)
     - walking around terrain

   This is on purpose. The net spends its capacity on the interesting
   question — "what should I be doing right now?" — instead of relearning
   arithmetic. It is why this trains in minutes instead of weeks.

   ---------------------------------------------------------------------
   WHAT ELSE IS IN A GENOME
   ---------------------------------------------------------------------
   Besides the network weights, each bot also carries:

     - skillPri: 4 numbers ranking Q/W/E/R, which decides its level-up
       order. Evolution discovers hero build orders on its own.
     - itemPri:  one number per item, which decides its shopping list.

   Those are not neural, they are just evolved preferences — and they
   turn out to matter enormously.
   ===================================================================== */
(function (root, factory) {
  // When this file is pasted inside lanebreaker.html, LB_INLINE_BRAIN is
  // defined just above it. That branch matters because the game script is
  // ALSO loaded in Node by the trainer, where `module` exists — without
  // this check the brain would overwrite the game's own module.exports.
  if (typeof LB_INLINE_BRAIN !== 'undefined') root.LBBrain = factory();
  else if (typeof module === 'object' && module.exports) module.exports = factory();
  else root.LBBrain = factory();
